...
if __name__ == "__main__":
    ...
    for k in k_list:
        ...
        argparse.add_argument("--m", default=2000, type=int)
        argparse.add_argument("--n", default=1000, type=int)
        argparse.add_argument("--k", default=k, type=int)
        argparse.add_argument("--noise", default=0.02, type=float)
        argparse.add_argument("--mask-prob", default=0.8, type=float)
        ...
        R_hat = pmf_solve(R * mask, mask, args.k, mu = 1e-2)
        print("== PMF")
        print("RMSE:", calc_unobserved_rmse(U, V, R_hat, mask))
        plot_image(R_hat, "pmf_solve.png")

        R_hat = biased_mf_solve(R * mask, mask, args.k, mu = 1e-2)
        print("== BMF")
        print("RMSE:", calc_unobserved_rmse(U, V, R_hat, mask))
